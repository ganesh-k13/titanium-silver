...
def uploadCode(inputJson):
    ...
    dcli = Docker_Client()
    thread = dcli.spawn_process(name='Input/'+file_name, num=random.randint(1, 99999999), params='%s 5000'%inputJson['USN'], path=app.config["CODE_FOLDER"], lang=META_DATA[inputJson["progLang"]]["container"], testcases=inputJson["testcases"])
    # Create a filename: 
    # opFileName = 
